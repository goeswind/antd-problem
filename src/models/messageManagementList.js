...
export default {
  namespace: 'messageManagementList',
  ...
  effects: {
    ...
    *saveQueryProData({ payload, callback }, { call, put,select }) {
      let response = yield select((state) => {return state.messageManagementList.queryListObj})
    ...
    *queryList({ payload, callback }, { call, put,select }) {
      const sendData = yield select((state) => {return state.messageManagementList.queryListObj})
