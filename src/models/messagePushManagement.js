...
export default {
  namespace: 'messagePushManagement',
  ...
  effects: {
    ...
    *saveQueryProData({ payload, callback }, { call, put,select }) {
      let response = yield select((state) => {return state.messagePushManagement.queryListObj})
    ...
    *queryList({ payload, callback }, { call, put,select }) {
      const sendData = yield select((state) => {return state.messagePushManagement.queryListObj})
