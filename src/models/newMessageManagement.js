...
export default {
  namespace: 'newMessageManagement',
  ...
  effects: {
    ...
    *saveQueryProData({ payload, callback }, { call, put,select }) {
      let response = yield select((state) => {return state.newMessageManagement.queryListObj})
    ...
    *queryList({ payload, callback }, { call, put,select }) {
      const sendData = yield select((state) => {return state.newMessageManagement.queryListObj})
